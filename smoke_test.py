...
from candidats.models import ProfilCandidat  # noqa: E402
...
n, _ = Utilisateur.objects.filter(email__in=['candidat@test.tn', 'admin@ensi.tn']).delete()
...
resp = c.post('/candidat/profil/', {
    'niveau_entree': 'bac', 'ville': 'Tunis', 'date_naissance': '2005-01-01',
    'type_diplome': 'math', 'specialite': '', 'etablissement': 'Lycee Pilote',
    'ville_etablissement': 'Tunis', 'annee_obtention': 2026,
...
# --- Conseiller ---
c2 = Client()
c2.login(email='conseiller@ensi-uma.tn', password='conseiller123')
check('GET espace conseiller', 200, c2.get('/conseil/espace/').status_code)
check('GET disponibilites', 200, c2.get('/conseil/disponibilites/').status_code)

# --- Admin ---
admin = Utilisateur.objects.create_superuser(
    email='admin@ensi.tn', password='admin12345', role='admin'
)
c3 = Client()
c3.login(email='admin@ensi.tn', password='admin12345')
check('GET dashboard admin', 200, c3.get('/tableau-de-bord/admin/').status_code)
check('GET /admin/', 200, c3.get('/admin/').status_code)
